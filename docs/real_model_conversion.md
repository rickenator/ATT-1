...
| M84 | Public-model q4 source comparison report: `--att1-q4` and `--q4-tol` flags in `compare_att1_to_source.py`; static weight comparison with `max_abs_error`, `mean_abs_error`, `topk_overlap`; cpu-q4 bench forward comparison against f32 reference; q4 tolerance policy documented; `check_q4_source_comparison()` smoke test. |
| M85 | Public-model q4 backend smoke validation: `compiler/validate_public_q4_backends.py`; cpu-q4 single/cluster pass; cuda-q4 explicitly unsupported; `fabric_packets_sent > 0` enforced for cluster; `check_public_q4_backend_smoke()` smoke test. |
