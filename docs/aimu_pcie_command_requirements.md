...
| M114 | AIMU fabric routing requirements | **Complete.** `docs/aimu_fabric_routing.md` added; 10-section routing specification: §1 what moves/stays local; §2 route descriptor fields (route_id, source_tile, dest_tile_mask, packet_type, payload_type, payload_bytes, fence_id, completion_fence_id, reduction_id, trace_id); §3 eight packet/route types (ACTIVATION_SEND, ACTIVATION_BROADCAST, PARTIAL_REDUCE, LOGITS_REDUCE, KV_TRANSFER, TILE_BARRIER, TRACE_EVENT, CONTROL_ACK) with Phase 1/2/3 capability table; §4 placement-driven routing topologies (layer-wise pipeline, row-split matmul, column-split matmul, head-wise attention, lm_head vocab-split, embedding lookup, replicated norms, KV ownership); §5 reduction semantics (sum/concat/pass-through, tile-ID-ordered determinism, f32/q8/q4 tolerance table); §6 fabric counters (per-tile packet/byte, per-packet-type, reduction, barrier, congestion/stall, error counters); §7 twelve validation rules F1–F12 (tile range, non-empty dest, nonzero payload, known type, reduction consistency, no missing route, acyclic DAG, barrier ordering, fence ordering, bandwidth compatibility, concat completeness, reduction participant completeness); §8 relationship to existing simulator (Phase 1 att1_fabric_t API mapping, cluster inference counters, M109 command plan integration, M115–M120 implementation path); §9 non-goals; §10 future milestone split M115–M120; `docs/aimu_architecture.md` §12 added; no C, Python, Makefile, binary format, or inference behavior changed |
