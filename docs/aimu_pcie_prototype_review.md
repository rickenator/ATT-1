...
| M131 | Userspace MMIO emulator regression suite | **COMPLETE** — `tests/test_aimu_mmio_regression.c`; 16 test functions covering 7 regression categories: (1) device probe — DEVICE_ID/REGISTER_MAP_VERSION/TILE_COUNT/GLOBAL_STATUS/per-tile metadata; (2) register behavior — RW round-trip, out-of-range/unaligned/RO-write error paths, reserved-offset sentinel 0xDEADBEEF; (3) RESET_CONTROL path — RESET_COUNTERS zeroes CNT_CMD_ISSUED_LO, WO register reads 0; (4) cmdq/doorbell — submit/ring/drain/completion, doorbell_write_count, FIFO order, QUEUE_FULL on saturated ring; (5) DMA descriptor path — valid H2D, zero-length/unaligned/out-of-range failures, dma_failed counter, huge tile_memory_bytes no-alloc; (6) EXEC replay (M130) — all 11 EXEC_*/KV_*/FABRIC_*/barrier/trace ops, per-op counters, unsupported op path via exec dispatcher and cmdq dispatcher; (7) replay integration — 6-command M129 plan through att1_aimu_host, UNSUPPORTED_OP signal, invalid tile_id rejected, summary fields; (8) trace snapshot determinism — two identical reset-separated replays produce identical cmdq counter snapshots; compile-time CUDA guard; no new source file; no inference/backend/CUDA/tokenizer/binary-format changes |
| M132 | Integrated execution/replay pipeline regression report | **COMPLETE** — `compiler/run_execution_replay_pipeline.py`; 6-stage end-to-end pipeline from tensor execution plan (M125) through command-plan mapping (M129), MMIO replay (M122), fabric route mapping (M117), route validation (M116), fabric route replay + bandwidth simulation (M123/M118); `compiler/test_execution_replay_pipeline.py` with 14 test functions (16 assertions); PIPELINE_VERSION=132; JSON report with 21 fields including `execution_plan_validation_status`, `command_plan_status`, `mmio_replay_status`, `fabric_route_status`, `fabric_replay_status`, `fabric_simulation_status`, `exec_commands_seen`, `required_fabric_gib_sec`, `fabric_status`, `final_status`; graceful binary-absent handling for stage 3; `_normalise("UNKNOWN")→"warn"` consistent with M119 `_STATUS_RANK`; `--strict` propagates to all sub-tools; `--workdir` preserves intermediate JSONs; `--tiles`, `--tile-memory-mib`, `--kv-memory-mib` forwarded to MMIO emulator; exit 0=pass/warn, 1=fail, 2=parse, 3=invalid flag; 16 PASS 0 FAIL (Python); 781 PASS 0 FAIL (C) maintained |
| M133 | Deterministic golden regression baselines | **COMPLETE** — `compiler/check_golden_regressions.py`; 12-check golden regression suite covering all ATT-1 AIMU planning and replay pipeline tools (g01–g11, g99); `compiler/fixtures/golden/` directory with 12 checked-in JSON baseline files (g01_placement_validation, g02_placement_advisory, g03_placement_scenario, g04_command_plan, g05_command_replay, g06_fabric_routes, g07_bw_simulation, g08_fabric_replay, g09_exec_plan_validation, g10_exec_to_commands, g11_exec_replay_pipeline, g99_malformed_exec_plan); `_Check` dataclass with chained workdir (g04→g05/g06, g06→g07/g08); `--check` (default) and `--update-golden` modes; `--fixture-dir` override; skip_if guards for binary-absent checks (g05, g11); negative golden g99 detects if malformed exec plan is ever accepted; extract functions isolate stable semantic fields (status, counts, command types, bytes) from volatile fields (timestamps, paths, elapsed times); exit 0=all pass, 1=one or more fail; 12 PASS 0 FAIL (Python golden check); 781 PASS 0 FAIL (C) maintained |
| M134 | Schema and version compatibility regression suite | **COMPLETE** — `compiler/check_schema_compat.py`; pure-Python schema compatibility checker for 5 schema types (placement/M98-M99, command plan/M109, fabric route/M115-M116, execution plan/M125-M128, pipeline/M132) plus cross-schema consistency; version constants mirror production tools; per-schema checks: version field type and value, required fields, list-type fields, count consistency, negative bytes, status enums; cross-schema: placement.header.tile_count vs command_plan.header.tile_count; unknown optional field policy: always W_UNKNOWN_FIELD warning (exit 0) in both default and strict modes; exit 0=pass/warn, 1=compat failure, 2=parse error; `compiler/test_schema_compat.py`: 31 tests covering all schema types, future/missing/type-mismatch/count-mismatch/negative-bytes/unknown-field/cross-schema cases; `compiler/fixtures/schema_compat/` with 21 checked-in fixtures; `docs/schema_compatibility.md` covering compatibility policy, required fields, unknown field policy, stable key names, deprecation process, version bump procedure; no C/Makefile/inference behavior changed; 31 PASS 0 FAIL (Python); 781 PASS 0 FAIL (C) maintained |
| M135 | Hostile-input and negative fixture expansion | **COMPLETE** — `compiler/check_hostile_inputs.py`; extended hostile-input checker (superset of M134) adding deeper validation: negative tile_count (E_NEGATIVE_TILE_COUNT), tile_count mismatch (E_TILE_COUNT_MISMATCH), duplicate tile_id/command_id/route_id/plan_command_id (E_DUPLICATE_ID), invalid non-int tile_id (E_INVALID_TILE_ID), q4 tensor missing group_size (E_Q4_MISSING_GROUP_SIZE), nonexistent owner_tile (E_NONEXISTENT_OWNER), unknown command_type/route_type/execution_phase (E_UNKNOWN_TYPE), negative byte counts in command plans (E_NEGATIVE_BYTE_COUNT), LOAD_TENSOR tensor command missing tensor_name (E_MISSING_TENSOR_NAME), unsupported dtype (E_UNSUPPORTED_DTYPE), zero payload on data routes (E_ZERO_PAYLOAD), reduction route without explicit reduction_behavior (E_MISSING_REDUCTION), invalid ordering_policy (E_INVALID_ORDERING), LOAD_TENSOR_TILE with no output_buffers (E_MISSING_OUTPUT_BUFFER), EXEC_MATMUL with no tensor_dependencies (E_MISSING_TENSOR_DEP), unknown expected_status (E_UNKNOWN_TYPE), pipeline commands_replayed > command_count (E_INCONSISTENT_COUNTS); imports base types from check_schema_compat.py; exit 0=pass/warn, 1=hostile failure, 2=parse error; `--strict`, `--report-json FILE` flags; `compiler/test_hostile_inputs.py`: 37 tests covering all 5 schema types hostile inputs + valid baselines + M133/M134 upstream regression guards + no-CUDA check + JSON report output + strict determinism; `compiler/fixtures/hostile/` with 27 checked-in hostile fixture files (6 placement + 6 command_plan + 6 fabric_route + 6 execution_plan + 3 pipeline); `docs/schema_compatibility.md` §11 hostile-input policy added (8 sub-sections: malformed must fail, negative counts, duplicate IDs, invalid enums, missing references, fabric semantic rules, count consistency, no silent fallback, fixture conventions); no C/Makefile/inference behavior changed; 37 PASS 0 FAIL (Python); 781 PASS 0 FAIL (C) maintained |
| M136 | Local full-regression runner | **COMPLETE** — `compiler/run_full_regression.py`; deterministic 8-step local regression runner executing ATT-1 validation layers in stable order: (1) make clean, (2) make (or make CUDA=1 with --cuda), (3) make test (or make test CUDA=1), (4) golden regressions (M133 check_golden_regressions.py), (5) schema compatibility (M134 test_schema_compat.py), (6) hostile-input regression (M135 test_hostile_inputs.py), (7) pipeline smoke (M132 run_execution_replay_pipeline.py), (8) git-tracked Python cache artifact check; human-readable summary table (step name, command, status, elapsed time); `--cuda` flag replaces build steps with CUDA=1 variants and marks run as CUDA signoff (RTX 3090-class required); `--no-build` skips steps 1–3 for fast re-runs; `--report-json FILE` writes machine-readable JSON report with mode/cuda/overall/elapsed_seconds/steps array; build failures abort immediately; exit 0=all pass, 1=one or more fail, 2=runner/tool error; `make regression` Makefile convenience target added; `compiler/test_full_regression.py`: 10 tests covering help exits 0, --no-build exits 0 on clean repo, JSON report valid structure, all 5 post-build step names present, overall field is string, no CUDA import in runner, no tracked Python cache artefacts, summary table present, failing step surfaces (sys.exit contract), step exit_codes are ints; no C/inference/backend behavior changed; 781 PASS 0 FAIL (C) maintained |
| M137 | CPU-only CI regression configuration | **COMPLETE** — `.github/workflows/ci.yml` added; GitHub Actions workflow triggered on push and pull_request; runs on ubuntu-latest; installs build-essential + python3 + make via apt (no CUDA, no NVIDIA driver, no model downloads); steps: (1) make clean, (2) make (CUDA=0 default; CUDA test targets compile with guards and skip bodies without GPU), (3) make test, (4) `python3 compiler/run_full_regression.py --no-build` (M133–M135 Python layers + M132 pipeline smoke), (5) git ls-files cache artifact guard (fail if any __pycache__/`.pyc`/`.pyo` tracked); no network except apt; CUDA signoff remains manual on RTX 3090-class host via `--cuda` flag; `docs/testing.md` added: five-section guide covering make test vs make regression vs CI, what each validates, what CI does not validate (CUDA kernel correctness, CUDA q4/q8/f32 runtime, public model inference, PCIe/MMIO hardware, RTX 3090 signoff), and comparison table; no C/Makefile/inference/backend/CUDA/tokenizer/binary-format behavior changed; 781 PASS 0 FAIL (C) maintained |
| M138 | CUDA signoff checklist and self-hosted runner plan | **COMPLETE** — `docs/CUDA_VALIDATION_PLAN.md` rewritten (M12 stub replaced); 8 sections: (1) CUDA validation policy (CPU CI is CPU-only; manual signoff on RTX 3090; milestone completion wording: "CPU validation complete; CUDA signoff pending" vs "CUDA validation complete on RTX 3090"; no silent fallback policy), (2) required manual signoff commands (make CUDA=1 + make test CUDA=1; run_full_regression.py --cuda; 5 bench smoke commands for f32/q8/q4 single and cluster), (3) CUDA paths to validate (f32: matmul/norm/rope/attention/ffn/infer/cluster/bench; q8: matmul/infer/cluster; q4: matmul/infer/cluster; backend matrix; q4/q8 golden comparison), (4) expected skip behavior (CPU-only build guards; CUDA unavailable explicit linker failure; no silent fallback policy), (5) self-hosted runner plan (RTX 3090 host requirements; runner labels: self-hosted/linux/x64/cuda/rtx3090; isolation rules; trigger strategy: workflow_dispatch + cuda-signoff-* tags), (6) example workflow reference, (7) signoff report template (date/host/GPU/driver/CUDA version/git commit/command output/pass-fail/notes), (8) non-goals; `.github/workflows/cuda-self-hosted.example.yml` added: inactive example workflow for RTX 3090 self-hosted runner (workflow_dispatch + cuda-signoff-* tag trigger; make CUDA=1 + make test CUDA=1 + run_full_regression.py --cuda --no-build; bench smoke steps with graceful model-absent skip; signoff JSON report uploaded as CI artifact; cache artifact guard); `docs/testing.md` §4 updated to reference CUDA_VALIDATION_PLAN.md and example workflow; `tests/test_aimu_exec.c` and `tests/test_aimu_mmio_regression.c` no_cuda_dep bug fixed (PASS on intentional CUDA=1 builds); no C/inference/backend/CUDA kernel/tokenizer/binary-format behavior changed; 781 PASS 0 FAIL (C) maintained |
| M139 | Release and outside-review readiness checklist | **COMPLETE** — `docs/RELEASE_READINESS.md` added; 11-section checklist: (1) repository hygiene (shell commands verifying no tracked __pycache__/.pyc/.pyo, no public model artifacts, no temp/editor files, no large accidental binaries; table of expected passing state), (2) build/test status (make clean+make+make test 781 PASS baseline; make regression all-five-layers PASS; CI green requirement; CUDA signoff status wording table), (3) CUDA validation policy summary (CPU CI ≠ CUDA validation; manual RTX 3090; self-hosted example reference; no silent fallback), (4) artifact policy table (tiny fixtures allowed; public model weights/generated .att1/source dirs/public tokenizer assets all external), (5) security/hostile-input policy (loader treats binary as hostile; schema validators reject malformed/future-version; no silent backend fallback; 37 hostile + 31 schema tests cited), (6) schema/version status (6 schemas: .att1 format, placement M98/M99, command plan M109, route report M115/M116, execution plan M125/M128, pipeline report M132; version rejection + W_UNKNOWN_FIELD policy), (7) current capability summary (21 capability rows), (8) non-goals table (8 rows: ASIC/PCIe/kernel driver/FPGA/cloud/mobile-Vulkan/patent claims/public model weights/real MMIO), (9) patent-sensitive handling (engineering wording rules; no claim language; private disclosures; grep audit command), (10) outside-review checklist (12 actionable checkbox items), (11) recommended next milestones M140–M145 table; `README.md` documentation section added (table linking 5 docs); `docs/testing.md` §6 added (pre-release quick sequence; link to RELEASE_READINESS.md); no C/Makefile/inference/backend/CUDA kernel/tokenizer/binary-format behavior changed; 781 PASS 0 FAIL (C) maintained |
| M140 | README and documentation navigation cleanup | **COMPLETE** — `README.md` fully rewritten (ATT-1 + AIMU summary paragraphs; current status table 12 rows; quick build/test; tiny fixture demo; documentation table with link to docs/INDEX.md; artifact policy; non-goals; annotated layout; future manuals section); `docs/INDEX.md` added: 11-category documentation index covering all 30 docs in docs/ by topic (Core ATT-1 runtime, model format/conversion, quantization, tokenizer/pretokenized input, placement/sizing/scenarios, AIMU architecture, AIMU PCIe/control plane, fabric routing, tensor execution planning, schema compatibility, testing/CI/CUDA signoff, release readiness, milestone history, images/diagrams); no C/Makefile/inference/backend/CUDA kernel/tokenizer/binary-format behavior changed; 781 PASS 0 FAIL (C) maintained |
| M141 | API/header ownership review | **COMPLETE** — `docs/api_ownership_review.md` added: 9-section review of all public ATT-1 headers (att1_model.h, att1_tensor.h, att1_backend.h, att1_infer.h, att1_cluster_infer.h, att1_quant.h, att1_shard.h, att1_status.h, att1_kv_cache.h, att1_kv_mmu.h, att1_tok_meta.h, att1_tok_ext.h, att1_shard_meta.h, AIMU headers); key findings: (1) two error systems (att1_status_t runtime vs att1_aimu_result AIMU wire codes; by design), (2) int-returning init/alloc APIs vs att1_status_t split (att1_tensor_alloc_f32/att1_kv_cache_init/att1_kv_mmu_init/att1_q8_matrix_alloc/att1_q4_matrix_alloc return int; deferred), (3) att1_infer_set_trace borrows trace (does not take ownership; trace must outlive infer while attached; was undocumented), (4) att1_infer_logits returns borrow valid only until next decode or destroy, (5) att1_infer_set_backend takes ownership (documented), (6) att1_kv_mmu struct fully exposed (ABI risk; opacification deferred to M148), (7) must-not-copy structs: att1_model/att1_tensor/att1_kv_cache/att1_kv_mmu/att1_q8_matrix/att1_q4_matrix/att1_shard_plan; narrow header comment fixes applied to all 7 structs + att1_infer_set_trace + att1_infer_logits; §4 const-correctness summary (all accessor functions correct; no missing const qualifiers); §5 error/status consistency; §6 thread-safety notes; §7 ABI/versioning notes; §8 recommended future fixes (7 items for M148); §9 change table; no behavior-changing changes; docs/INDEX.md updated; 781 PASS 0 FAIL (C) maintained |
