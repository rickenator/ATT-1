...
- Milestone 167: Two-tile emulated cluster decode (complete) — [include/att1_aimu_cluster_bridge.h](include/att1_aimu_cluster_bridge.h)/[src/aimu_cluster_bridge.c](src/aimu_cluster_bridge.c) add a transport-agnostic host-side relay built out of the already-frozen fabric send/receive/barrier calls, bridging activation/logits packets and barrier arrivals between two independent `att1_aimu_conformance_endpoint` instances whether in-process or two separate `att1-aimu-endpoint` daemon processes over sockets; [tests/test_aimu_cluster_decode.c](tests/test_aimu_cluster_decode.c) proves a real two-layer, two-tile transformer decode (activation routing, barrier rendezvous, row-parallel partial-logit reduction) in-process against a cpu-f32 reference, and the same fabric/barrier protocol over two real daemon processes connected via sockets; real `EXEC_*` compute execution across the socket transport remains a documented gap (raw host pointers in the M166 exec hook are not valid across process boundaries), deferring the backend-comparison-report `pcie` column.
...
- Milestone 167: Two-tile emulated cluster decode (complete) — `att1_aimu_cluster_bridge` relays activation/logits packets and barrier arrivals between two independent conformance endpoints (in-process or two real daemon processes over sockets); proves real two-tile transformer decode with activation routing, barrier rendezvous, and partial-logit reduction in-process, and the same protocol over real sockets; cross-process `EXEC_*` compute (and the `pcie` comparison-report column) remains future work.
...
- Milestone 172: Beachhead workload definition and baseline metrics (complete) — [compiler/validate_m172_beachhead.py](compiler/validate_m172_beachhead.py) runs a deterministic two-tile q8 cluster decode workload repeatedly, reports latency stability, memory movement per token, KV pressure, and optional cost-per-million-token comparison against a CUDA baseline; [tests/test_bench_smoke.c](tests/test_bench_smoke.c) covers the harness on checked-in tiny fixtures via `--allow-tiny-fixture`.
- Milestone 173: Placement and capacity validation at 256 MB / 512 MB / 1 GB device budgets (complete) — [compiler/validate_m173_capacity.py](compiler/validate_m173_capacity.py) evaluates M98/M100 placement reports against the fixed Phase 2 per-tile envelopes, scales KV by context/session targets, emits a machine-readable M175 capacity decision artifact, and resolves the M93 §8.15-2 KV page-size question by selecting 16-token KV pages under the current f32 KV model; [tests/test_bench_smoke.c](tests/test_bench_smoke.c) covers the gate on the checked-in placement fixture.
- Milestone 174: Activation precision study (complete) — [compiler/validate_m174_activation_precision.py](compiler/validate_m174_activation_precision.py) compares f32 versus bf16 inter-tile activation packets from fabric route reports, measures deterministic bf16 round-trip error, and records the M93 §8.15-3 decision: bf16 activation payloads are acceptable for the Phase 2 prototype planning path while f32 remains the reference/diagnostic precision.
