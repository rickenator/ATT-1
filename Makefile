...
AIMU_TRANSPORT_BENCH_BIN := $(BUILD_DIR)/att1-aimu-transport-bench
...
TEST_NAMES := smoke tensor matmul rmsnorm softmax rope silu swiglu \
	...
	cuda_matmul cuda_norm cuda_ffn cuda_rope cuda_attention cuda_transformer_block cuda_infer cuda_cluster cuda_bench \
     ...
     aimu_mmio_regression aimu_endpoint aimu_endpoint_fault_injection backend_pcie aimu_cluster_decode aimu_replay_fidelity aimu_transport_bench
...
AIMU_TRANSPORT_BENCH_OBJS := $(BUILD_DIR)/tools/att1-aimu-transport-bench.o $(COMMON_OBJS)
...
all: $(SIM_BIN) $(INSPECT_BIN) $(BENCH_BIN) $(Q8_BENCH_BIN) $(SIZE_BIN) $(AIMU_REPLAY_BIN) $(AIMU_REPLAY_FIDELITY_BIN) $(AIMU_TRANSPORT_BENCH_BIN) $(AIMU_EMULATOR_BIN) $(AIMU_MMIO_REPLAY_BIN) $(AIMU_ENDPOINT_BIN) $(TINY_LLM_BIN) $(CLUSTER_LLM_BIN)
...
$(AIMU_TRANSPORT_BENCH_BIN): $(AIMU_TRANSPORT_BENCH_OBJS)
	$(CC) $(CFLAGS) $(LDFLAGS) -o $@ $^ $(LDLIBS)
...
test: $(INSPECT_BIN) $(BENCH_BIN) $(Q8_BENCH_BIN) $(SIZE_BIN) $(AIMU_REPLAY_BIN) $(AIMU_REPLAY_FIDELITY_BIN) $(AIMU_TRANSPORT_BENCH_BIN) $(AIMU_EMULATOR_BIN) $(AIMU_MMIO_REPLAY_BIN) $(AIMU_ENDPOINT_BIN) $(TEST_BINS)
	@for test_bin in $(TEST_BINS); do \
		./$$test_bin || exit $$?; \
	done

test-verbose: $(INSPECT_BIN) $(BENCH_BIN) $(Q8_BENCH_BIN) $(SIZE_BIN) $(AIMU_REPLAY_BIN) $(AIMU_REPLAY_FIDELITY_BIN) $(AIMU_TRANSPORT_BENCH_BIN) $(AIMU_EMULATOR_BIN) $(AIMU_MMIO_REPLAY_BIN) $(AIMU_ENDPOINT_BIN) $(TEST_BINS)
	@for test_bin in $(TEST_BINS); do \
		printf '\n=== %s ===\n' "$$test_bin"; \
		./$$test_bin || exit $$?; \
	done
