...
def check_milestone_consistency(repo: Path, report: CheckReport) -> None:
    ...
    Validate key milestone/status invariants:
      - OPERATION_LOG must contain a Milestone 166 entry.
      - OPERATION_LOG must show M165 as complete.
    ...
    if op_log.exists():
        ...
        # M166 should appear (at minimum in "Next Prompt for Codex" or as an entry)
        if not re.search(r"Milestone\s+166|M166", text):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG does not mention Milestone 166 (M166 not started or missing)",
            )

        # M165 should be listed as complete (have a "Milestone 165:" entry)
        if not re.search(r"^- Milestone 165:", text, re.MULTILINE):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG has no '- Milestone 165:' entry (M165 not yet recorded as complete)",
            )
