...
Checks performed:
  ...
  2. Required docs — all 17 key documents from docs/INDEX.md and README.md
  ...
  4. Milestone/status consistency — OPERATION_LOG mentions M152;
     M151 marked complete; M146/M147 manuals not still "pending".
...
def check_milestone_consistency(repo: Path, report: CheckReport) -> None:
    ...
    Validate key milestone/status invariants:
      - OPERATION_LOG must contain a Milestone 152 entry.
      - OPERATION_LOG must show M151 as complete.
    ...
    if op_log.exists():
        ...
        # M152 should appear (at minimum in "Next Prompt for Codex" or as an entry)
        if not re.search(r"Milestone\s+152|M152", text):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG does not mention Milestone 152 (M152 not started or missing)",
            )

        # M151 should be listed as complete (have a "Milestone 151:" entry)
        if not re.search(r"^- Milestone 151:", text, re.MULTILINE):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG has no '- Milestone 151:' entry (M151 not yet recorded as complete)",
            )
