...
Checks performed:
  ...
  4. Milestone/status consistency — OPERATION_LOG mentions M175;
     M174 marked complete; M146/M147 manuals not still "pending".
...
def check_milestone_consistency(repo: Path, report: CheckReport) -> None:
    ...
    if op_log.exists():
        ...
        # M175 should appear (at minimum in "Next Prompt for Codex" or as an entry)
        if not re.search(r"Milestone\s+175|M175", text):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG does not mention Milestone 175 (M175 not started or missing)",
            )

        # M174 should be listed as complete (have a "Milestone 174:" entry)
        if not re.search(r"^- Milestone 174:", text, re.MULTILINE):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
