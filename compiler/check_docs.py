...
Checks performed:
  ...
  2. Required docs — all key documents from docs/INDEX.md and README.md
  ...
  4. Milestone/status consistency — OPERATION_LOG mentions M149;
     M148 marked complete; M146/M147 manuals not still "pending".
...
def check_milestone_consistency(repo: Path, report: CheckReport) -> None:
    ...
    Validate key milestone/status invariants:
      - OPERATION_LOG must contain a Milestone 149 entry.
      - OPERATION_LOG must show M148 as complete.
    ...
    if op_log.exists():
        ...
        # M149 should appear (at minimum in "Next Prompt for Codex" or as an entry)
        if not re.search(r"Milestone\s+149|M149", text):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG does not mention Milestone 149 (M149 not started or missing)",
            )

        # M148 should be listed as complete (have a "Milestone 148:" entry)
        if not re.search(r"^- Milestone 148:", text, re.MULTILINE):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG has no '- Milestone 148:' entry (M148 not yet recorded as complete)",
            )
