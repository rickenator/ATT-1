...
Checks performed:
  ...
  4. Milestone/status consistency — OPERATION_LOG mentions M150;
     M149 marked complete; M146/M147 manuals not still "pending".
...
def check_milestone_consistency(repo: Path, report: CheckReport) -> None:
    ...
    Validate key milestone/status invariants:
      - OPERATION_LOG must contain a Milestone 150 entry.
      - OPERATION_LOG must show M149 as complete.
    ...
    if op_log.exists():
        ...
        # M150 should appear (at minimum in "Next Prompt for Codex" or as an entry)
        if not re.search(r"Milestone\s+150|M150", text):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG does not mention Milestone 150 (M150 not started or missing)",
            )

        # M149 should be listed as complete (have a "Milestone 149:" entry)
        if not re.search(r"^- Milestone 149:", text, re.MULTILINE):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG has no '- Milestone 149:' entry (M149 not yet recorded as complete)",
            )
