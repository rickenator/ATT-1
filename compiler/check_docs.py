...
Checks performed:
  ...
  4. Milestone/status consistency — OPERATION_LOG mentions M153;
     M152 marked complete; M146/M147 manuals not still "pending".
...
def check_milestone_consistency(repo: Path, report: CheckReport) -> None:
    ...
    Validate key milestone/status invariants:
      - OPERATION_LOG must contain a Milestone 153 entry.
      - OPERATION_LOG must show M152 as complete.
    ...
    if op_log.exists():
        ...
        # M153 should appear (at minimum in "Next Prompt for Codex" or as an entry)
        if not re.search(r"Milestone\s+153|M153", text):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG does not mention Milestone 153 (M153 not started or missing)",
            )

        # M152 should be listed as complete (have a "Milestone 152:" entry)
        if not re.search(r"^- Milestone 152:", text, re.MULTILINE):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG has no '- Milestone 152:' entry (M152 not yet recorded as complete)",
            )
