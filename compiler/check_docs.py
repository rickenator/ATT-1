...
def check_milestone_consistency(repo: Path, report: CheckReport) -> None:
    ...
    Validate key milestone/status invariants:
      - OPERATION_LOG must contain a Milestone 167 entry.
      - OPERATION_LOG must show M166 as complete.
    ...
    if op_log.exists():
        ...
        # M167 should appear (at minimum in "Next Prompt for Codex" or as an entry)
        if not re.search(r"Milestone\s+167|M167", text):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG does not mention Milestone 167 (M167 not started or missing)",
            )

        # M166 should be listed as complete (have a "Milestone 166:" entry)
        if not re.search(r"^- Milestone 166:", text, re.MULTILINE):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG has no '- Milestone 166:' entry (M166 not yet recorded as complete)",
            )
