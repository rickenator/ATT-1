...
Checks performed:
  ...
  4. Milestone/status consistency — OPERATION_LOG mentions M151;
     M150 marked complete; M146/M147 manuals not still "pending".
...
def check_milestone_consistency(repo: Path, report: CheckReport) -> None:
    ...
    Validate key milestone/status invariants:
      - OPERATION_LOG must contain a Milestone 151 entry.
      - OPERATION_LOG must show M150 as complete.
    ...
    if op_log.exists():
        ...
        # M151 should appear (at minimum in "Next Prompt for Codex" or as an entry)
        if not re.search(r"Milestone\s+151|M151", text):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG does not mention Milestone 151 (M151 not started or missing)",
            )

        # M150 should be listed as complete (have a "Milestone 150:" entry)
        if not re.search(r"^- Milestone 150:", text, re.MULTILINE):
            report.add_error(
                "milestone-consistency",
                "docs/OPERATION_LOG.md",
                "OPERATION_LOG has no '- Milestone 150:' entry (M150 not yet recorded as complete)",
            )
