...
static int att1_kv_mmu_validate_address(att1_kv_mmu *mmu,
                                        uint64_t session_id,
                                        size_t layer_id)
{
    if (mmu == NULL) {
...
static int att1_kv_mmu_validate_position(att1_kv_mmu *mmu, size_t position)
{
    if (mmu == NULL) {
        return -1;
    }
...
static int att1_kv_mmu_validate_range(att1_kv_mmu *mmu,
...
int att1_kv_mmu_init(att1_kv_mmu *mmu,
                     const att1_kv_mmu_config *config)
{
    size_t i = 0u;

    if ((mmu == NULL) || !att1_kv_mmu_config_valid(config)) {
        return -1;
    }
...
void att1_kv_mmu_free(att1_kv_mmu *mmu)
{
    size_t i = 0u;
...
int att1_kv_mmu_create_session(att1_kv_mmu *mmu, uint64_t session_id)
{
    size_t i = 0u;
...
int att1_kv_mmu_destroy_session(att1_kv_mmu *mmu, uint64_t session_id)
{
...
int att1_kv_mmu_append(att1_kv_mmu *mmu,
                       uint64_t session_id,
                       size_t layer_id,
                       size_t position,
                       const float *key,
                       const float *value)
{
    ...
    if (att1_kv_mmu_validate_address(mmu, session_id, layer_id) != 0) {
        return -1;
    }

    if (att1_kv_mmu_validate_position(mmu, position) != 0) {
        return -1;
    }
...
int att1_kv_mmu_read(att1_kv_mmu *mmu,
                     uint64_t session_id,
                     size_t layer_id,
                     size_t head_id,
                     size_t position,
...
int att1_kv_mmu_copy_range(att1_kv_mmu *mmu,
                           uint64_t session_id,
                           size_t layer_id,
                           size_t head_id,
                           size_t start_position,
                           ...
                           float *out_values)
{
    ...
    if (att1_kv_mmu_validate_range(mmu,
                                   start_position,
                                   position_count) != 0) {
        return -1;
    }
...
int att1_kv_mmu_lookup_page(att1_kv_mmu *mmu,
                            uint64_t session_id,
                            size_t layer_id,
                            size_t position,
