...
static int att1_block_q8_weights_valid(
...
{
    att1_attention_config attention_config;
    att1_attention_q8_weights attention_weights;
    float *attention_input = NULL;
    float *attention_output = NULL;
    float *attention_residual = NULL;
    float *ffn_input = NULL;
    ...
    if (!att1_block_q8_weights_valid(weights) ||
        !att1_block_config_valid(config, cache)) {
        return -1;
    }
    ...
    if (att1_attention_forward_backend_q8(attention_output,
                                          cache,
                                          attention_input,
                                          &attention_weights,
                                          &attention_config,
...
{
    att1_attention_config attention_config;
    att1_attention_q4_weights attention_weights;
    float *attention_input = NULL;
    float *attention_output = NULL;
    float *attention_residual = NULL;
    float *ffn_input = NULL;
    ...
    if ((weights == NULL) ||
        (weights->attention_norm == NULL) || (weights->ffn_norm == NULL) ||
        (weights->wq == NULL) || (weights->wk == NULL) ||
        (weights->wv == NULL) || (weights->wo == NULL) ||
        (weights->w_gate == NULL) || (weights->w_up == NULL) ||
        (weights->w_down == NULL)) {
        return -1;
    }

    if (!att1_block_config_valid(config, cache)) {
        return -1;
    }
    ...
    if ((attention_input == NULL) || (attention_output == NULL) ||
        (attention_residual == NULL) || (ffn_input == NULL) ||
        (ffn_gate == NULL) || (ffn_up == NULL) ||
        (ffn_hidden == NULL) || (ffn_output == NULL)) {
        goto cleanup;
    }
    ...
    if (att1_attention_forward_backend_q4(attention_output,
                                          cache,
                                          attention_input,
                                          &attention_weights,
                                          &attention_config,
    ...
    att1_add_f32(attention_residual, input, attention_output, config->model_dim);
